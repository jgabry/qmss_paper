...
\caption{$v^{\it DEM}$  vs. $\pi^{DEM}$}
\label{fig:log_vratio_vs_ptdem}
\end{figure}


Figure~\ref{fig:log_vratio_vs_ptdem} shows $v^{\it DEM}$ plotted against $\pi^{DEM}$. The shape of the curve is similar to the standard seats-votes curve  
...
The trends of $\log{(v^{\it RATIO})}$ and $\pi^{MAJ}$ over time are shown in the visual summary of the data set in Figure~\ref{fig:data_summary}. \\[10pt]
