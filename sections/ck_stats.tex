...
\end{figure}
   ...
   However, the penalty matrix $\mathbf{P}$ corresponding to an undirected $RW_\kappa$ prior over parameter vector $\lambda \in \mathbb{R}^C$ has rank $C - \kappa$ and the resulting precision matrix $\mathbf{Q} = \mathbf{P}/\tau^2$  is not positive definite.\footnote{By definition it is then not a precision matrix, but the term will be used for simplicity.}  While in the $RW_1$ case this be overcome by applying a slight jitter to the elements of $\mathbf{P}$, for $\kappa > 1$ it requires splitting $\lambda$ into $C - \kappa$ free parameters and $\kappa$ {\it pinned} parameters, where the values of the pinned parameters can be computed from the {\it free} parameters using the properties of the multivariate normal distribution. 
