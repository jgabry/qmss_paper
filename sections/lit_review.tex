...
geographic regions are asked if they are in favor of a particular policy. One approach for estimating
the level of underlying support for the policy in each region would be to estimate separate
models for the $R$ regions. However, it is also likely that there is some relationship between 
regions that should be incorporated into the joint probability model. 

Let $y_n = 1$ if the $n$th respondent supports the policy in question and $0$ otherwise, and let $r[n]$ 
denote the region for the $n$th individual/observation. For simplicity, assume that there is a single 
individual-level predictor $x$ (e.g. income, age, etc.) and a single region-level predictor $u$ 
(e.g. average income). The hierarchical model

%
\begin{align*}
 {\rm logit}\left(P(y_n = 1) \right) &= \alpha_{r[n]} + \beta_{r[n]} x_n, \qquad \text{for } n = 1, \dots, N \\[5pt]
 \alpha_r &= \gamma^\alpha_0 + \gamma^\alpha_1 u_r + \epsilon^\alpha_r, \quad \text{ for } r = 1, \dots, R \\[5pt]
  \beta_r &= \gamma^\beta_0 + \gamma^\beta_1 u_r + \epsilon^\beta_r, \qquad\qquad \vdots  \\[5pt]
   \epsilon^\alpha_r &\sim N(0, \tau^2_\alpha) \\[5pt]
   \epsilon^\beta_r &\sim N(0, \tau^2_\beta).
\end{align*}

\noindent is what \citeA{gelman_arm_2007} refer to as a {\it varying-intercept}, 
{\it varying-slope model}. That is, both the intercepts and the slopes vary by group (region, in this example) 
and are given probability models which depend on further levels of parameters (the hyperparameters). 

The model above estimates $R$ intercepts ($\alpha_1, \dots, \alpha_R$) and $R$ coefficients 
...
by the data for other groups as well \shortcite{jackman_bayesian_2009}.\footnote{
...
and used to estimate the group-specific parameters $\alpha_r$ and $\beta_r$.} That is, {\it strength} is 
borrowed from all groups when estimating the parameter(s) corresponding to each individual group. As a consequence the  estimates for the group-level parameters are {\it shrunk} toward each other, 
with the amount of shrinkage depending on how far each is from the overall mean.\footnote{The term
...
and Katznelson and in this thesis. See \citeA{gelman_arm_2007}, \citeA{gelman_bayesian_2013} and 
\citeA{jackman_bayesian_2009} for more thorough and formal introductions to the topic of hierarchical modeling.  
...
and spatial statistics, however GMRFs appear only rarely in quantitative social science, and \citeA{wawro_designing_2014} is the first example extending the applications of GMRFs to historical 
...
$\partial^{\mtrx{A}}_1 = \{v_2, v_6\}$ and $\partial^{\mathbf{B}}_1 = \{v_2, v_3\}$.  
...
for each element in $\theta$ there is a corresponding node in $V$, and no edge $e_{ij}$ between 
nodes $v_i$ and $v_j$ \emph{if and only if} $\theta_i$ and $\theta_j$ are conditionally independent 
given all other elements of $\theta$ \shortcite{rue_gaussian_2005}.\footnote{$v_i \notin 
