...
\citeA{wawro_designing_2014} and then draws on the relevant literature 
...
or overfit the data \shortcite{gelman_bayesian_2013}. Hierarchical (or multilevel) modeling provides a 
...
For example, consider the simple scenario in which survey respondents from many different
...
support for $P$ in, say, New England is entirely uninformative about support for $X$ in all 
...
For the $n$th observation, let $r[n]$ denote the respondent's region and $\theta_n = P(y_n = 1)$ the probability
the respondent expresses support for policy $P$. For simplicity, assume that there is a single individual-level 
predictor $x$ (e.g. income) and a single region-level predictor $w$ (e.g. average income). 
The hierarchical model

%
\begin{align*}
\theta_n &= {\rm logit}^{-1}\left(\alpha_{r[n]} + \beta_{r[n]} x_n\right), \quad \text{for } n = 1, \dots, N \\[5pt]
...
{\it varying-slope model}. That is, both the intercepts and the slopes -- the effects associated with the variable $x$-- vary by group (region, in this example) and are given probability models which depend on further levels of parameters (the hyperparameters). 
...
be modeled -- however, in practice the data are informative only up to a point (and there are 
...
by the data for other groups as well.\footnote{
The assumption of exchangeability is important here, but a sufficient discussion of exchangeability 
is beyond the scope of this thesis. See \citeA{gelman_bayesian_2013}.} This is commonly referred 
to as {\it borrowing strength} or {\it shrinkage}. In the example above, for instance, the 
estimates for each $\alpha_r$ are informed not only by the data from region $r$ but 
also by the observations from all other regions through the common hyperparameters.\footnote{
In other words, the population-level hyperparameters are estimated from the data for all groups 
and used to estimate the group-specific parameters $\alpha_r$ and $\beta_r$.} This describes
...
borrowed from all groups when estimating the parameter(s) corresponding to each individual group. As a 
consequence the  estimates for the group-level parameters are {\it shrunk} toward each other, 
with the amount of shrinkage depending on how far each is from the overall mean \shortcite{jackman_bayesian_2009, lipsky_shrinkage_2010}.\footnote{The term
{\it shrinkage} is a bit of a misnomer. The values of the parameters themselves are not necessarily smaller, 
but rather the distance from the common mean is smaller.}

The term {\it hierarchical} is a suitable designation for a wide (theoretically unlimited) family of 
models, from the simple example above to the semi-parameteric STAR models used by Wawro 
and Katznelson and in this thesis. See \citeA{lipsky_shrinkage_2010} for an excellent introduction to shrinkage 
in multilevel modeling, and \citeA{gelman_arm_2007} and \citeA{jackman_bayesian_2009} for more thorough and formal treatments of hierarchical modeling. 
...
In \textsc{graph $\mtrx{C}$}, the node $v_{11}$ and its neighborhood $\partial^\mtrx{C}_{11}$ are
highlighted. For a more comprehensive discussion of undirected graphs in the context of GMRFs 
see \citeA{rue_gaussian_2005}. 
...
\begin{tikzpicture}
\node[obs, color=Sepia, text=Thistle] (r1) {$\nu_1$};
\node[obs, below left=of r1, color=Sepia, text=Thistle] (r2) {$\nu_2$};
\node[obs, below=of r2, color=Sepia, text=Thistle] (r3) {$\nu_3$};
\node[obs, below right=of r3, color=Sepia, text=Thistle] (r4) {$\nu_4$};
\node[obs, above right=of r4, color=Sepia, text=Thistle] (r5) {$\nu_5$};
\node[obs, above=of r5, color=Sepia, text=Thistle] (r6) {$\nu_6$};
\edge [-, color=Thistle, bend right=30] {r1} {r2} ;
\edge [-, color=Thistle, bend left=30] {r1} {r6} ;
\edge [-, color=Thistle] {r2} {r3} ;
\edge [-, color=Thistle, bend right=30] {r3} {r4} ;
\edge [-, color=Thistle, bend right=30] {r4} {r5} ;
\edge [-, color=Thistle] {r5} {r6} ;
\end{tikzpicture}
%
 \hspace{1cm} 
 %
\begin{tikzpicture}
\node[obs, color=ProcessBlue, text = black] (r1) {$\nu_1$};
\node[obs, below right=of r1, color=ProcessBlue, text = black] (r2) {$\nu_2$};
\node[obs, below right=of r2,  color=ProcessBlue, text = black] (r3) {$\nu_3$};
\node[obs, below right=of r3,  color=ProcessBlue, text = black] (r4) {$\nu_4$};
...
\edge [-, color=gray] {r7} {r8} ; \edge [-, color=PineGreen] {r7} {r11} ;
\edge [-, color=gray] {r8} {r12} ; 
\edge [-, color=gray] {r9} {r10} ; \edge [-, color=PineGreen] {r10} {r11} ; \edge [-, color=PineGreen] {r11} {r12} ; 

\edge [-, color=gray] {r4} {r7} ; \edge [-, color=gray] {r2} {r7} ; 
\edge [-, color=gray] {r5} {r10} ; \edge [-, color=PineGreen] {r6} {r11} ; 
\edge [-, color=gray, bend right=30] {r4} {r12} ; \edge [-, color=PineGreen, bend left=30] {r1} {r11} ;

\end{tikzpicture}
\vspace{.5cm}
\caption{Examples of simple undirected graphs. In \textsc{graph A} each node has two neighbors. In \textsc{graph B} node $\nu_4$ has only a single neighbor while the others each have two neighbors. In \textsc{graph C} the node $\nu_{11}$ and its neighborhood $\partial^\mtrx{G}_{11}$ are highlighted.}
...
obey the requisite Markov properties describing pairwise, local, and global conditional independencies \shortcite{rue_gaussian_2005}.} 
...
special cases \shortcite{fahrmeir_bayesian_2001}.\footnote{The varying-intercept, varying-slope models described in \ref{hierarchical} are examples of mixed effects models and are also special cases of the STAR model.} 
...
\citeA{brezger_generalized_2006}, can conveniently be expressed as the matrix product 
...
\citeA{fahrmeir_bayesian_2001} suggest several possible choices for a prior on a smooth f
...
simultaneous estimation of a smoothing function $f_j$ and the amount of smoothness.
 \citeA{fahrmeir_bayesian_2001} and \citeA{brezger_bayesx:_2005} recommend  
