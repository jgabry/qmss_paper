...
Given observations $y$ and covariates $w_{1}, \dots, w_{K}$, the generalized linear model (GLM) 
...
\noindent to the conditional mean of the outcome $\mu_n = E(y_n | w_{n1}, \dots, w_{nK})$ via a link 
...
For instance, the vary-intercept, varying-slope model in section~\ref{hierarchical} uses the logit link 
function to map between the real numbers and the $[0,1]$ interval on which probabilities are defined.  

The structured additive regression (STAR) model replaces the linear predictor $\zeta$ of the 
...
special cases \shortcite{fahrmeir_bayesian_2001}.\footnote{\citeA{brezger_star_2005} provide a 
...
and edges $E$ connecting vertices corresponding to neighboring (or proximate) regions. A prior on 
a parameter vector $\theta = (\theta_1, \dots, \theta_R)$ can be constructed such that the matrix 
