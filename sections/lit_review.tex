...
geographic regions are asked if they are in favor of a particular policy $X$. One approach for estimating
...
support for $X$ across regions, which should be incorporated into the joint probability model. In 
the United States, for example, estimating separate models would impose the assumption that
support for $X$ in, say, New England is entirely uninformative about support for $X$ in all 
other regions of the country. A hierarchical framework allows for a more realistic model in which 
support for $X$ can vary across regions without disregarding interregional dependence.      

Let $y_n = 1$ if the $n$th respondent supports policy $X$ and $0$ otherwise, and let $r[n]$ 
...
with the amount of shrinkage depending on how far each is from the overall mean \shortcite{jackman_bayesian_2009}.\footnote{The term
