...
The assumption of exchangeability is also important here, but a sufficient discussion of this subtle topic 
...
\end{forest}
\caption{A simple hierarchical structure with $N$ observations $y_1, \dots, y_N$ organized in to $J$ groups, with group-level parameters $\theta_1, \dots, \theta_J$ and global hyperparameter(s) $\phi$. The set $\{y_{n} : {j[n] = j}\}$ represents the collection of observations belonging to group $j$.}
...
are given probability models conditional on a further level of parameters 
(the hyperparameters). 
...
in group $j$ but also indirectly by the observations from all other groups due to the common 
dependence on the global parameter $\phi$. In the survey example, the estimates for each 
...
function of the distance from that mean \shortcite{jackman_bayesian_2009, lipsky_shrinkage_2010}.
