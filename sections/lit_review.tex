...
%\begin{figure}
%\begin{align*}
%y_n &\sim \mathcal{N}(\theta_{j[n]}, \sigma^2_{j[n]}), \quad \text{for } n = 1, \dots, N \\
%\theta_j &\sim \mathcal{N}(\mu, \tau^2), \quad \text{for } j = 1, \dots, J \\
%(\mu, \tau^2) &\sim p(\mu, \tau^2)
%\end{align*}
%\caption{Example of a simple hierarchical model.}
%\end{figure}
%
%\begin{figure}
%%\begin{tikzpicture}
%%\node[const] (y1) {$y_1, y_2, y_3$};
%%\node[const, below=of y1] (theta1) {$\theta_1$};
%%\node[const, right=of y1] (y2) {$y_4, y_5, y_6$};
%%\node[const, below=of y2] (theta2) {$\theta_2$};
%%\node[const, right=of y2] (ydots) {$\dots$};
%%\node[const, below=of ydots] (thetadots) {$\dots$};
%%\node[latent, right=of ydots] (yN1) {$y_{N-5}, y_{N-4}, y_{N-3}$};
%%\node[const, below=of yN1] (thetaJ1) {$\theta_{J-1}$};
%%\node[const, right=of yN1] (yN) {$y_{N-2}, y_{N-1}, y_N$};
%%\node[const, below=of yN] (thetaJ) {$\theta_J$};
%%\node[const, below=of thetadots] (mu) {$\mu, \tau^2$};
%%%\node[const, right =of mu] (tau) {$\tau^2$};
%%\factoredge [->, color=black] {theta1} {y1} ;
%%\edge [->, color=black]  {thetaJ} {yN};
%%\edge [->, color=black] {theta2} {y2} ;
%%\edge [->, color=black]  {thetaJ1} {yN1};
%%\edge [->, color=black] {thetadots} {ydots} ;
%%\edge [->, color=black] {mu} {theta1} ;
%%\edge [->, color=black] {mu} {theta2} ;
%%\edge [->, color=black] {mu} {thetaJ1} ;
%%\edge [->, color=black] {mu} {thetaJ} ;
%%\edge [->, color=black] {mu} {thetadots} ;
%%\end{tikzpicture}
%\begin{forest}
%for tree={
%  edge={->,>=latex},
...
%CREATE FIGURE SHOWING HIERARCHY with ys on top with each connected below to a theta, and then all thetas connected to mu and tau below%
...
is beyond the scope of this thesis. See \citeA{gelman_bayesian_2013}.} The joint posterior distribution for all parameters then factors nicely as
...
refer to the resulting models as {\it multilevel models} \shortcite{gelman_arm_2007}.  For example, 
...
structures present in the data as well as computational considerations \shortcite{gelman_bayesian_2013}. 
...
In the first example above, inference for $\theta_j$ is directly informed by the observations in
...
\shortcite{jackman_bayesian_2009, lipsky_shrinkage_2010}.

See \citeA{gelman_arm_2007} and \citeA{jackman_bayesian_2009} for more 
thorough and formal treatments of hierarchical modeling. Also see \citeA{lipsky_shrinkage_2010} 
