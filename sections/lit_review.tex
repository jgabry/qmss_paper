...
with group-level parameters $\theta = (\theta_1, \dots, \theta_J)$ and global parameter(s) $\xi$ 
...
\end{forest}
\caption{A simple hierarchical structure with $N$ observations $y_1, \dots, y_N$ organized in to $J$ groups, with group-level parameters $\theta_1, \dots, \theta_J$, and global hyperparameter(s) $\xi$. The notation $\{y_{n : j[n] = j}\}$ is used for the set of observations belonging to the $j$th group.}
...
are also estimated from the data and thus require prior distributions (hyperpriors) to be specified. In theory there is no limit to the number 
